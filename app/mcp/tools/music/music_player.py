...
class MusicPlayer:
    ...
    def __init__(self):
        # 使用Qt音乐播放器替代pygame
        self.qt_player = get_music_player()
    ...
    def _sync_state_from_qt_player(self):
        """
        从Qt播放器同步状态
        """
        try:
            self.current_song = self.qt_player.current_song
            self.current_url = self.qt_player.current_url
            self.song_id = self.qt_player.song_id
            self.total_duration = self.qt_player.total_duration
            self.is_playing = self.qt_player.is_playing
            self.paused = self.qt_player.paused
            self.current_position = self.qt_player.current_position
            self.start_play_time = self.qt_player.start_play_time
            self.lyrics = self.qt_player.lyrics
            self.current_lyric_index = self.qt_player.current_lyric_index
        except Exception as e:
    ...
    async def play_pause(self) -> dict:
        ...
        try:
            if not self.is_playing and self.current_url:
                ...
                # 使用Qt播放器的暂停/恢复功能
                result = await self.qt_player.pause_resume()
                self._sync_state_from_qt_player()
                return result
    ...
    async def stop(self) -> dict:
        ...
        try:
            ...
            # 使用Qt播放器停止
            result = await self.qt_player.stop()
            self._sync_state_from_qt_player()
            return result
    ...
    async def seek(self, position: float) -> dict:
        ...
        try:
            ...
            # 使用Qt播放器跳转
            result = await self.qt_player.seek(position)
            self._sync_state_from_qt_player()
            return result
