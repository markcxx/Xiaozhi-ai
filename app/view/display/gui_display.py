...
class GuiDisplay(BaseDisplay, QObject, metaclass=CombinedMeta):
    ...
    def _on_text_sent(self, text: str):
        """
        文本发送事件.
        """
        if self.send_text_callback:
            try:
                import asyncio
                if asyncio.iscoroutinefunction(self.send_text_callback):
                    asyncio.create_task(self.send_text_callback(text))
                else:
                    self.send_text_callback(text)
            except Exception as e:
                self.logger.error(f"文本发送回调执行失败: {e}")
    ...
    async def update_status(self, status: str, connected: bool):
        ...
        # 通过homeInterface更新状态
        if hasattr(self.root, 'homeInterface'):
            self.root.homeInterface.updateStatus(status, connected)
    ...
    async def update_text(self, text: str):
        ...
        # 通过homeInterface更新文本
        if hasattr(self.root, 'homeInterface'):
            self.root.homeInterface.updateText(text)
    ...
    async def update_emotion(self, emotion_name: str):
        ...
        if hasattr(self.root, 'homeInterface'):
            try:
                self.root.homeInterface.updateEmotion(emotion_name)
            except Exception as e:
                self.logger.error(f"设置表情时发生错误: {str(e)}")
    ...
    def _connect_events(self):
        ...
        """
        if hasattr(self.root, 'homeInterface'):
            # 连接homeInterface的信号到回调函数
            home = self.root.homeInterface
            if hasattr(home, 'voiceCard'):
                home.voiceCard.manualPressed.connect(self._on_manual_button_press)
                home.voiceCard.manualReleased.connect(self._on_manual_button_release)
                home.voiceCard.autoClicked.connect(self._on_auto_button_click)
                home.voiceCard.abortClicked.connect(self._on_abort_button_click)
                home.voiceCard.modeClicked.connect(self._on_mode_button_click)
            
            if hasattr(home, 'textCard'):
                home.textCard.textSent.connect(self._on_text_sent)
    ...
    def _setup_system_tray(self):
        ...
        try:
            ...
            if os.getenv("XIAOZHI_DISABLE_TRAY") == "1":
                ...
                return
            from app.view.views.components.system_tray import SystemTray
    ...
    def _on_settings_button_click(self):
        ...
        """
        try:
            from app.view.views.settings import SettingsWindow
